...
def status_print(ip,port,thread_id,rps,path_get):
    print(f"\r{Fore.YELLOW}FLOODING{Fore.LIGHTWHITE_EX}HTTP {Fore.WHITE}=> {Fore.BLUE}TARGET{Fore.WHITE}={ip}:{port} {Fore.YELLOW}PATH{Fore.WHITE}={path_get} {Fore.CYAN}id{Fore.WHITE}={Fore.YELLOW}{thread_id}{Fore.RESET}") 
    print(f"\r{Fore.BLUE}FLOODING{Fore.YELLOW}HTTP {Fore.WHITE}=> {Fore.YELLOW}TARGET{Fore.BLUE}={ip}:{port} {Fore.GREEN}PATH{Fore.WHITE}={path_get} {Fore.CYAN}Rps{Fore.WHITE}={Fore.RED}{rps} {Fore.RESET}") 
